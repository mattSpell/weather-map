...
(function(){
  ...
  function weather(data){
    chartsData[place] = {};
    chartsData[place].dataProvider = [];
    $('#graph').append(`<div class=graph data-zip=${place}></div>`);
    for(var i = 0; i < 10; i++){
      var high = data.forecast.simpleforecast.forecastday[i].high.fahrenheit * 1;
      var low = data.forecast.simpleforecast.forecastday[i].low.fahrenheit * 1;
      var day = data.forecast.simpleforecast.forecastday[i].date.weekday;
      chartsData[place].dataProvider.push({'high': high, 'low': low, 'day': day});
    }
    createGraph();
    charts[place].validateData();
  }

  function createGraph(){
    //debugger;
    let graph = $(`.graph[data-zip=${place}]`)[0];
    charts[place] = AmCharts.makeChart(graph, {
          'type': 'serial',
          'theme': 'none',
